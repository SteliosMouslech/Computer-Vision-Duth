...
source = cv2.imread("N2.png", cv2.IMREAD_GRAYSCALE)
dst = cv2.copyMakeBorder(source, 1, 1, 1, 1, cv2.BORDER_CONSTANT, None, 0)

final = dst[:]
window = [0] * 9
for y in range(1, dst.shape[0] - 1):
    for x in range(1, dst.shape[1] - 1):
        window[0] = dst[y - 1, x - 1]
        window[1] = dst[y, x - 1]
        window[2] = dst[y + 1, x - 1]
        window[3] = dst[y - 1, x]
        window[4] = dst[y, x]
        window[5] = dst[y + 1, x]
        window[6] = dst[y - 1, x + 1]
        window[7] = dst[y, x + 1]
        window[8] = dst[y + 1, x + 1]
        window.sort()
        final[y, x] = window[4]
final = final[1:final.shape[0] - 1, 1:final.shape[1] - 1]

cv2.imshow('source', final)  # Show the image
cv2.waitKey(0)

thresh, final_Binary= cv2.threshold(final, 57, 255, cv2.THRESH_BINARY)

cv2.imshow('Final_Picture1_Binary', final_Binary)  # Show the image
cv2.waitKey(0)

kernel = cv2.getStructuringElement(cv2.MORPH_CROSS, (3, 3))
final_transformed = cv2.morphologyEx(final_Binary, cv2.MORPH_OPEN, kernel,iterations=3)
cv2.imshow('Final_Picture12_', final_transformed)  # Show the image
cv2.waitKey(0)

final_transformed = cv2.morphologyEx(final_transformed, cv2.MORPH_CLOSE, kernel,iterations=1)
